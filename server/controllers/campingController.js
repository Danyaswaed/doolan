...
const listTables = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error listing database tables', error: error.message });
  }
};
...
const getCampingSpotById = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching camping spot', error: error.message });
  }
};
