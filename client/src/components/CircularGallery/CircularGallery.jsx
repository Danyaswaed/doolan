...
class App {
  constructor(container, { items, bend, textColor = "#ffffff", borderRadius = 0, font = "bold 30px Figtree" } = {}) {
    ...
    this.autoDirection = 1; // 1 = right, -1 = left
    this.autoSpeed = 0.2; // Speed of automatic movement
    this.createRenderer()
    this.createCamera()
    this.createScene()
    this.onResize()
  ...
  onTouchDown(e) {
    ...
    // Pause automatic movement when dragging
  ...
  onTouchUp() {
    this.isDown = false
    // Resume automatic movement when dragging ends
  ...
  update() {
    // Automatic movement
    this.scroll.target += this.autoDirection * this.autoSpeed;

    // Check if we need to change direction
    if (this.medias && this.medias[0]) {
      const halfGalleryLength = this.medias[0].width * this.medias.length / 2;
      if (Math.abs(this.scroll.target) > halfGalleryLength) {
        this.autoDirection *= -1; // Change direction
      }
    }
